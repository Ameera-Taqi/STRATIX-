...
export class HealthBreakdownComponent {
  ...
  readonly items = computed(() => {
    ...
    return [
      {
        key: 'progress',
        labelKey: 'health.factorProgress',
        display: `${f.progress}%`,
        tone: 'text-emerald-600 dark:text-emerald-300',
      },
      {
        key: 'onTime',
        labelKey: 'health.factorOnTimeRate',
        display: `${f.onTimeRate}%`,
        tone: 'text-emerald-600 dark:text-emerald-300',
      },
      {
        key: 'delayed',
        labelKey: 'health.factorDelayedRate',
        display: `${f.delayedRate}%`,
        tone: 'text-red-600 dark:text-red-300',
      },
      {
        key: 'blocked',
        labelKey: 'health.factorBlockedRate',
        display: `${f.blockedRate}%`,
        tone: 'text-red-600 dark:text-red-300',
      },
      {
        key: 'critical',
        labelKey: 'health.factorCriticalPenalty',
        display: `−${f.criticalRiskPenalty}`,
        tone: 'text-red-600 dark:text-red-300',
      },
    ];
  });
